...
from ILCourtScraper.Extra.time import callSleep, currTime, time
...
class Scraper:
    ...
    def __init__(self, num_of_crawlers=0):
        self.logger = Logger('Scraper.log', getPath(N=2) + f'logs{sep}').getLogger()
        self.db = DB(logger=self.logger).getDB('SupremeCourt')
